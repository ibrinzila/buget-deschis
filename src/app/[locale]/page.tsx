  ...
  return (
    <div style={{ position: 'relative', background: 'var(--paper)', border: '1px solid var(--ink)', padding: '24px' }}>
      ...
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        <g>
          <text x={xs[xs.length - 1] + 10} y={y(d[d.length - 1].approved) + 4} fontFamily="var(--serif)" fontSize="14" fontWeight="500" fill="var(--forest)">100.0</text>
          <text x={xs[xs.length - 1] + 10} y={y(d[d.length - 1].actual) + 4} fontFamily="var(--serif)" fontSize="13" fill="var(--ink-3)">94.2</text>
          <text x={xs[xs.length - 1] + 10} y={y(d[d.length - 1].euFunds) + 4} fontFamily="var(--serif)" fontSize="13" fill="var(--ochre-2)" fontWeight="500">9.4</text>
        </g>
        <g>
          <line x1={xs[3]} x2={xs[3]} y1={y(d[3].euFunds) + 4} y2={y(d[3].euFunds) + 40} stroke="var(--ink-3)" strokeWidth="0.5" />
          <rect x={xs[3] - 70} y={y(d[3].euFunds) + 40} width="180" height="40" fill="var(--paper-2)" stroke="var(--ink-3)" strokeWidth="0.5" />
          <text x={xs[3] - 62} y={y(d[3].euFunds) + 56} fontFamily="var(--serif)" fontStyle="italic" fontSize="12" fill="var(--ink)">{t('annotationTitle')}</text>
          <text x={xs[3] - 62} y={y(d[3].euFunds) + 70} fontFamily="var(--sans)" fontSize="11" fill="var(--ink-3)">{t('annotationSub')}</text>
        </g>
      </svg>
    </div>
  );
  ...
  return (
    <div>
          ...
          <div className="bd-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: '60px', alignItems: 'end' }}>
            <div>
              ...
              <p className="rise rise-2" style={{ fontSize: '19px', lineHeight: 1.5, color: 'var(--ink-2)', maxWidth: '560px', margin: '0 0 36px' }}>
                {th('subtitle')} <strong style={{ color: 'var(--ink)', fontWeight: 600 }}>{th('subtitleEm')}</strong>
              </p>
            ...
            <div className="rise rise-3 bd-hero-aside" style={{ borderLeft: '1px solid var(--ink)', paddingLeft: '32px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 24px' }}>
                <MiniStat label={th('kpiExecuted')} value={<><CountUp to={94.2} decimals={1} /> mld</>} sub={th('kpiExecutedSub')} color="var(--forest)" />
                <MiniStat label={th('kpiProcurement')} value="41.8 mld" sub={th('kpiProcurementSub')} color="var(--ochre)" />
                <MiniStat label={th('kpiObi')} value="81/100" sub={th('kpiObiSub')} />
                <MiniStat label={th('kpiEu')} value="9.4 mld" sub={th('kpiEuSub')} />
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: 'var(--paper-2)', padding: '72px 0', borderTop: '1px solid var(--ink)', borderBottom: '1px solid var(--ink)' }}>
        <div className="wrap">
          <Eyebrow num="§02">{trnd('eyebrow')}</Eyebrow>
          <div className="bd-trend-head" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '48px', alignItems: 'end', marginBottom: '32px' }}>
            <h2 className="serif" style={{ fontSize: 'var(--fs-h1)', lineHeight: 1.05, fontWeight: 500, letterSpacing: '-0.02em', margin: 0 }}>
              {trnd('titlePre')} <em style={{ color: 'var(--forest)' }}>{trnd('titleEm')}</em> {trnd('titlePost')}
            </h2>
            <p style={{ fontSize: '15px', lineHeight: 1.6, color: 'var(--ink-2)', margin: 0 }}>{trnd('lead')}</p>
          </div>
      ...
      <section style={{ background: 'var(--paper)', padding: '72px 0', borderTop: '1px solid var(--ink)' }}>
        <div className="wrap">
          ...
          <div className="bd-highlights" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0, borderTop: '1px solid var(--ink)' }}>
            <HighlightBlock num="01" stat={thl('one.stat')} label={thl('one.label')} body={thl('one.body')} />
            <HighlightBlock num="02" stat={thl('two.stat')} label={thl('two.label')} body={thl('two.body')} />
            <HighlightBlock num="03" stat={thl('three.stat')} label={thl('three.label')} body={thl('three.body')} last />
          </div>
        </div>
      </section>
